...
class HCPDataValidator:
    ...
    def setup_ui(self):
        ...
        # License Issues Tab
        self.license_tab = tk.Frame(self.notebook, bg='white')
        ...
        self.candidate_tab = tk.Frame(self.notebook, bg='white')
        ...
        self.affiliation_tab = tk.Frame(self.notebook, bg='white')
    ...
    def load_file(self):
        ...
        if file_path:
            ...
            self.status_label.config(text=f"Loading file: {os.path.basename(file_path)}...")
            self.root.update()
    ...
    def run_analysis(self):
        ...
        self.status_label.config(text="Running validation checks...")
    ...
    def perform_analysis(self):
        try:
            ...
            if self.check_license.get():
                self.root.after(0, lambda: self.status_label.config(text="Checking licenses..."))
                license_results = self.check_licenses()
                self.results['license'] = license_results
            
            # 2. CANDIDATE CHECK
            if self.check_candidate.get():
                self.root.after(0, lambda: self.status_label.config(text="Checking candidate records..."))
                candidate_results = self.check_candidates()
                self.results['candidate'] = candidate_results
            
            # 3. AFFILIATION CHECK
            if self.check_affiliation.get():
                self.root.after(0, lambda: self.status_label.config(text="Checking affiliations..."))
